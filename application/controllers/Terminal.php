                ...
                if ($change > $cashBalance) {
                    ...
                    if ($updateDrawer) {
                        ...
                        if ($createTransaction === true) {
                            ...
                            // This block is removed to prevent calling $printer->close() again
                            // $printer->cut();
                            // $printer->close();
                            ...
                            try {
                                ...
                                function formatLine($left, $right, $maxLength = 64, $margin = 5) {
                                    $leftMargin = str_repeat(' ', $margin);
                                    $rightMargin = str_repeat(' ', $margin);
                                    
                                    $leftLength = strlen($left);
                                    $rightLength = strlen($right);
                                    $spaces = $maxLength - $leftLength - $rightLength - (2 * $margin);
                                
                                    return $leftMargin . $left . str_repeat(' ', max($spaces, 0)) . $right . $rightMargin . "\n";
                                }
                                $printer->setJustification(Printer::JUSTIFY_LEFT);

                                $printer->selectPrintMode(Printer::MODE_FONT_B);

                                $printer->text(formatLine("Philippine International", "Date: " . date('m-d-Y')));

                                $printer->text(formatLine("Convention Center", date("H:i:s A")));
                                $printer->selectPrintMode(Printer::MODE_FONT_B);
                                $printer->feed();

                                $OR = $companyData['OR'] + 1;
                                $access = $this->input->get('access_type');
                                $code =  $this->input->get('parking_code');
                                $gateIn = date('Y-m-d H:i:s A', $this->input->get('unix_entry_time'));
                                $paytime = date('Y-m-d H:i:s A', $this->input->get('parking_time'));
                                $vid = $this->input->get('vehicle_class');
                                $vcat = "";
                                if($vid == 1){
                                    $vcat = "Motorcyle";
                                }else if ($vid == 2)
                                ...
                                $printer->text("MIN: 1234567891". $companyData['MIN'] . "\n");
                                $printer->text("S/I #: ". $OR . "\n");
                                $printer->text($access . ": ". $code . "\n");
                                $printer->text("Vehicle Class: ". $vcat . "\n");
                                // $printer->selectPrintMode(Printer::MODE_FONT_B);
                                $printer->text("\n\nInvoice Sales\n");
                                // $printer->selectPrintMode(Printer::MODE_FONT_B);
                                $printer->setJustification(Printer::JUSTIFY_CENTER);
                                ...
                                $printer->selectPrintMode(Printer::MODE_FONT_B);
                                $printer->text(formatLine("Cashier: ", $username));
                                $printer->setJustification(Printer::JUSTIFY_CENTER);
                                $printer->text("================================================================\n");
                                $printer->text(formatLine("Gate In: ", $gateIn));
                                $printer->text(formatLine("Bill Time: ", $paytime));
                                $printer->setJustification(Printer::JUSTIFY_CENTER);
                                $printer->selectPrintMode();
                                $printer->feed();
                                ...
                                // Handle errors (e.g., printer not found)
