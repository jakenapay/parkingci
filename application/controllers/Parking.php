...
//		ob_end_clean();
	...
	public function create_dummy()
	{
		...
		$pictureName = '';
		$picturePath = '';
		$inTime = strtotime('-1 hour'); // Set to 1 hour ago



		// Generate and insert 20 dummy records
		for ($i = 0; $i < 10; $i++) {
			$randomCode = strtoupper(substr(md5(mt_rand()), 0, 5)); // Generate a random 5-character alphanumeric string
			$parkingCode = "ACB{$randomCode}";

			$data = [
				'GateId' => $gateId,
				'AccessType' => 'Plate', // Plate, QR, RFtag
				'parking_code' => $parkingCode,
				'vechile_cat_id' => $vehicleCatId,
				'rate_id' => $rateId,
				'slot_id' => $slotId,
				'pictureName' => $pictureName,
				'picturePath' => $picturePath,
				'in_time' => $inTime, // Always an hour ago
				'out_time' => '',
				'paid_time' => '',
				'total_time' => '',
				'earned_amount' => '',
			...
			$this->model_parking->create($data);
		}

		$this->session->set_flashdata('success', '20 Dummy records successfully created');
		redirect('parking/', 'refresh');
	}
