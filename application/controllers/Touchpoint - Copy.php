            ...
            if ($cashDrawer) {
                ...
                if ($paymode == "Cash") {
                    ...
                    if ($discountType == "none") {
                        ...
                        $OriginalvatableSale = round($originalAmount / VATRATE,2);                        
                        $OriginalVat = ($OriginalvatableSale * VATPERCENT);
                        $discountCode = $this->input->post('discount_type');
                        $discounts = $this->model_touchpoint->getDiscounts($discountCode, $vehicleId);
                        $discountPercentage = (int)$discounts['percentage']/100;
                        $totalDiscount = ($OriginalvatableSale * $discountPercentage);
                        $discountAmount = $originalAmount - $totalDiscount;
                        ...
                        if($discounts['vat_exempt'] == 1){
                            $totalVat = 0;
                            $vatExempt = $OriginalVat ;
                            $totalSales = $discountAmount - $vatExempt;
                            $vatableSale=0;
                        }
                        else{
                            $totalVat = $OriginalVat;
                            $vatExempt = 0;                            
                            $vatableSale=$OriginalvatableSale ;
                            $totalSales = $discountAmount + $vatableSale;
                        } 
                        // echo "Discounts: " . $discountType . "</br>";
                        // echo "Original Amount: " . number_format($originalAmount, 2) . "</br>";
                        // echo "Original VAT sales: " . number_format($OriginalvatableSale,2) . "</br>";
                   ...
                   // $bill = $this->input->post('amount');
                    ...
                    $bill= strval(round($totalSales)*100 );
                      ...
                      //  $totalSales = $this->input->post("total_sales");
                        ...
                        if($postTransaction){
                            ...
                            if($discountType == 'senior'){
                                $discType = "SC";
                                $discPercent = ($this->input->post("vehicleClass")) == 2 ? "20%" : "10%";
                                ...
                                $discPercent = ($this->input->post("vehicleClass")) == 2 ? "10%" : "5%";
