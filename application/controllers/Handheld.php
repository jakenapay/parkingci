...
class Handheld extends CI_Controller
{
	public function __construct()
	{
		...
		// $this->load->model("model_handheld");
	}
	...
	public function RequestClientBill(){
		$access = $this->input->post("atype");
		$pcode = $this->input->post("pcode");


		$dataResponse = $this->model_handheld->getData($access, $pcode);


		if($dataResponse){
			$checkInTime = $dataResponse['in_time'];
            $checkoutTime = strtotime('now');
            $totalMin = ceil((abs($checkoutTime - $checkInTime) / 60));
            $totalHour = floor((abs($checkoutTime - $checkInTime) / 60) / 60);
            $min = ((abs($checkoutTime - $checkInTime) / 60) % 60);

            $vehicleId = $dataResponse['vechile_cat_id'];
            $rates = $this->model_handheld->getRate($vehicleId);
			...
			$response = array(
				'status' => 'success',
				'message' => 'Client entry found',
				'parkingId' => $dataResponse['id'],
				'gate' => $dataResponse['GateId'],
				'accessType' => $dataResponse['AccessType'],
				'parkingCode' => $dataResponse['parking_code'],
				'parkingStay' => $consumeParkingHour,
				'amountDue' => $amount,
				'paymentTime' => $checkoutTime,
				'parkingStatus' => $vrate,
				'vehicleClass' => $vehicleId,
				'entryTime' => $dataResponse['in_time']
			);
		}else{
			$response = array (
				'status' => 'failed',
	...
	public function CreateBillTransaction(){
        ...
        // $ptuId = 2;
		$ptuId = $this->input->get('terminalId');
        ...
        $transaction = array(
            ...
            'min' => $company['min'],
            'sn' => $ptu['sn'],
            'ptu_num' => $ptuData['BIR_SN'],
            'ptu_date' => $ptuData['issued_date'],
        );
	...
	public function RequestQrphTransaction(){
		$bill = $this->input->get('amount');
	...
	public function VerifyQrphTransaction(){
