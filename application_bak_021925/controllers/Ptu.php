...
*/
	...
	{	
		...
		$data = array(
			...
			'at_time' => date("Y-m-d h:i:sa", strtotime('now'))
		);		
	...
	public function Barrieron()
        {
                $response = file_get_contents("http://10.0.0.149:8080/control_on");
                if ($response === FALSE) {
                        echo "Error occurred while sending GET request.";
                } else
                {
                        echo "Response received: " . $response;
                }
	}
	public function Barrieroff()
        {
                $response = file_get_contents("http://10.0.0.149:8080/control_off");
                if ($response === FALSE) {
                        echo "Error occurred while sending GET request.";
                } else
                {
